...
class AgentDDPGCuriosityImagination():
    ...
    def train_model(self):
        ...
        _, entropy_t     = self._imagination(state_t)
    ...
    def _imagination(self, state_t):
        ...
        features_t   = torch.zeros((self.rollouts, self.batch_size, self.features_count)).to(state_t.device)

        for r in range(self.rollouts):
            action_t, _     = self._sample_action(state_t, self.epsilon)
            
            features_t[r]   = self.model_forward(state_t, action_t)
            actions_t[r]    = action_t

        #transpose to shape batch, rollouts, features_count
        features_t  = features_t.transpose(0, 1)

        #transpose to shape batch, rollouts, actions_count
        actions_t = actions_t.transpose(0, 1)

        #features standart deviation
        std_features = features_t.std(dim=1).mean(dim=1)

        #actions standart deviation
        std_actions = actions_t.std(dim=1).mean(dim=1)

        entropy_t = self.beta_entropy*torch.tanh(std_features/(std_actions + 0.0001)).detach()
        
        return features_t, entropy_t