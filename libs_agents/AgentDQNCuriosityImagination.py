...
class AgentDQNCuriosityImagination():
    def __init__(self, env, ModelDQN, ModelForward, ModelForwardTarget, ModelReward, Config):
        self.env    = env
    ...
    def main(self, show_activity = False):
        ...
        q_probs_np     = numpy.exp(q_values_np - numpy.max(q_values_np))
        q_probs_np     = q_probs_np/numpy.sum(q_probs_np)

        _, rewards_t        = self._imagination(state_t)
        rewards_np          = rewards_t.squeeze(0).detach().to("cpu").numpy()
        rewards_probs_np    = numpy.exp(q_probs_np - numpy.max(q_probs_np))
        rewards_probs_np    = rewards_probs_np/numpy.sum(rewards_probs_np)

        action = self._sample_action(q_probs_np*rewards_probs_np, self.epsilon)
    ...
    def train_model(self):
        ...
        action_one_hot      = self._action_one_hot(action_t) 
        reward_predicted_t  = self.model_reward(state_t, action_one_hot)
        reward_target_t     = reward_t + curiosity_t

        loss_reward         = (reward_target_t.detach() - reward_predicted_t)**2
    ...
    def _imagination(self, state_t):
        rollouts = self.actions_count

        states_t    = torch.zeros((rollouts, ) + self.state_shape).to(state_t.device)
        actions_t   = torch.zeros((rollouts, self.actions_count)).to(state_t.device)
        for r in range(rollouts):
            actions_t[r]    = 1.0
            states_t[r]     = state_t

        rewards_t = self.model_reward(states_t, actions_t)
           
        return actions_t, rewards_t
