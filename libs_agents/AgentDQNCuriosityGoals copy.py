...
class AgentDQNCuriosityGoals():
    def __init__(self, env, ModelDQN, ModelForward, ModelForwardTarget, ModelCuriosityPredictor, Config):
        self.env    = env
        ...
        self.experience_replay  = ExperienceBufferGoals(config.experience_replay_size, self.state_shape, self.actions_count, self.sequence_length)
        ...
        self.model_goal_creator      = ModelCuriosityPredictor.Model(self.state_shape, self.sequence_length)
        self.optimizer_curiosity_predictor  = torch.optim.Adam(self.model_goal_creator.parameters(), lr=config.learning_rate_goal_creator)

        self.state              = env.reset()
        self.goal_buffer        = self._init_goal_buffer(self.state)
    ...
    def main(self, show_activity = False):
        ...
        goal_buffer_t  = self._add_goal_buffer(self.state)
        goal_t         = self.model_goal_creator(goal_buffer_t)
        ...
        if self.enabled_training:   
            curiosity_t = self._curiosity(state_t, action_one_hot_t)
            motivation  = self.beta*curiosity_t.squeeze(0).detach().to("cpu").numpy()

            self.experience_replay.add(self.state, self.action, self.reward, done, motivation)
    ...
    def train_model(self):
        state_t, state_next_t, states_seq_t, goals_t, action_t, reward_t, done_t, _ = self.experience_replay.sample(self.batch_size, self.model_dqn.device)
        ...
        #train goal creating model, MSE loss
        goals_predicted_t   = self.model_goal_creator(states_seq_t)
        loss_goal           = (goals_t.detach() - goals_predicted_t)**2
        loss_goal           = loss_goal.mean()
        
        self.optimizer_curiosity_predictor.zero_grad()
        loss_goal.backward()
        self.optimizer_curiosity_predictor.step()

        goals_predicted_t   = goals_predicted_t.detach()

        #q values, state now, state next
        q_predicted      = self.model_dqn.forward(state_t, goals_predicted_t)
        q_predicted_next = self.model_dqn_target.forward(state_next_t, goals_predicted_t)
        ...
        self.loss_goal              = (1.0 - k)*self.loss_goal        + k*loss_goal.detach().to("cpu").numpy()
        self.internal_motivation    = (1.0 - k)*self.internal_motivation + k*curiosity_t.mean().detach().to("cpu").numpy()

        #print(">>> ", self.loss_forward, self.loss_goal, self.internal_motivation)
    ...
    def save(self, save_path):
        ...
        self.model_goal_creator.save(save_path + "trained/")

    def load(self, save_path):
        self.model_dqn.load(save_path + "trained/")
        self.model_forward.load(save_path + "trained/")
        self.model_forward_target.save(save_path + "trained/")
        self.model_goal_creator.load(save_path + "trained/")

    def get_log(self):
        result = "" 
        result+= str(round(self.loss_forward, 7)) + " "
        result+= str(round(self.loss_goal, 7)) + " "
    ...
    def _init_goal_buffer(self, state):
        return numpy.tile(state, self.sequence_length).reshape((self.sequence_length,) + state.shape)

    def _add_goal_buffer(self, state):
        state_ = numpy.expand_dims(state, axis=0)
        
        #remove last row, append new state on top
        tmp = numpy.delete(self.goal_buffer, (-1), axis = 0)
        self.goal_buffer = numpy.vstack([state_, tmp])

        return torch.from_numpy(self.goal_buffer).to(self.model_goal_creator.device).unsqueeze(0).float()
