...
class AgentPPOCuriosityMulti():
    def __init__(self, envs, ModelPPO, ModelsForward, ModelsForwardTarget, Config):
        self.envs = envs
        ...
        self.models_forward             = []
        self.models_forward_target      = []
        self.optimizers_forward         =[]

        for m in range(len(ModelsForward)):
            model_forward          = ModelsForward[m].Model(self.state_shape, self.actions_count)
            optimizer_forward      = torch.optim.Adam(model_forward.parameters(), lr=config.learning_rate_forward)
            model_forward_target   = ModelsForwardTarget[m].Model(self.state_shape, self.actions_count)

            self.models_forward.append(model_forward)
            self.optimizers_forward.append(optimizer_forward)
            self.models_forward_target.append(model_forward_target)
        ...
        self.models_usage_probs = numpy.ones(len(ModelsForward))/len(ModelsForward)
    ...
    def main(self):
        ...
        curiosity_t, curiosity_model_id     = self._curiosity(states_t, action_one_hot_t)
        ...
        models_usage_probs          = self._models_usage_probs(curiosity_model_id)
        self.models_usage_probs     = (1.0 - k)*self.models_usage_probs + k*models_usage_probs
    ...
    def save(self, save_path):
        ...
        for m in range(len(self.models_forward)):
            self.models_forward[m].save(save_path + "trained/" + str(m) + "_")
            self.models_forward_target[m].save(save_path + "trained/" + str(m) + "_")

    def load(self, load_path):
        self.model_ppo.load(load_path + "trained/")
        
        for m in range(len(self.models_forward)):
            self.models_forward[m].load(load_path + "trained/" + str(m) + "_")
            self.models_forward_target[m].load(load_path + "trained/" + str(m) + "_")
    ...
    def get_log(self):
        ...
        for i in range(len(self.models_usage_probs)):
            result+= str(round(self.models_usage_probs[i], 7)) + " "
    ...
    def train(self): 
        ...
        for e in range(self.training_epochs):
            for batch_idx in range(batch_count):
                ...
                #train forward models, MSE loss
                loss_forward_sum = 0.0
                for m in range(len(self.models_forward)):
                    state_norm_t = states - self.states_running_mean_t

                    features_predicted_t    = self.models_forward[m](state_norm_t, action_one_hot_t)
                    features_target_t       = self.models_forward_target[m](state_norm_t, action_one_hot_t)

                    loss_forward = (features_predicted_t.detach() - features_target_t)**2
                    loss_forward = loss_forward.mean()

                    self.optimizers_forward[m].zero_grad()
                    loss_forward.backward()
                    self.optimizers_forward[m].step()

                    loss_forward_sum+= loss_forward.detach().to("cpu").numpy()

                k = 0.02
                self.loss_forward  = (1.0 - k)*self.loss_forward + k*loss_forward_sum/len(self.models_forward)
    ...
    def _curiosity(self, state_t, action_one_hot_t):
        ...
        curiosity_result = torch.zeros((len(self.models_forward), state_t.shape[0])).to(self.model_ppo.device)
        ...
        for m in range(len(self.models_forward)):
            ...
            if len(curiosity_t.shape) > 1:
                curiosity_t    = curiosity_t.view(curiosity_t.shape[0], -1).max(dim=1)[0]

            curiosity_result[m] = curiosity_t

        curiosity_result_max    = curiosity_result.max(dim=0)[0]
        curiosity_model_id      = curiosity_result.argmax(dim=0).detach().to("cpu").numpy()


        return curiosity_result_max, curiosity_model_id
    ...
    def _models_usage_probs(self, curiosity_model_id):        
        unique, counts = numpy.unique(curiosity_model_id, return_counts=True)
    
        result = numpy.zeros(len(self.models_forward))

        for id, count in zip(unique, counts):
            result[id] = count


        result = result/(result.sum() + 0.0000001)        
        return result