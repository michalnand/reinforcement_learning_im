...
class AgentDQNCuriosityGoals():
    def __init__(self, env, ModelDQN, ModelForward, ModelForwardTarget, ModelGoalCreator, Config):
        self.env    = env
        ...
        self.model_goal_creator         = ModelGoalCreator.Model(self.state_shape, self.sequence_length)
        self.optimizer_goal     = torch.optim.Adam(self.model_goal_creator.parameters(), lr=config.learning_rate_goal_creator)

        self.state              = env.reset()

        self.goal_buffer        = numpy.zeros((self.sequence_length, ) + self.state_shape)
    ...
    def _init_goal_buffer(self, state):
        self.goal_buffer = numpy.tile(state, self.sequence_length).reshape((self.sequence_length,) + state.shape)

    def _add_goal_buffer(self, state):
        #remove last row, append new state on top
        tmp = numpy.delete(self.goal_buffer, (-1), axis = 0)
        self.goal_buffer = numpy.vstack([state, tmp])
    ...
    def main(self, show_activity = False):
        ...
        goal_t         = self.model_goal_creator(goal_buffer_t).detach()
        ...
        if self.enabled_training:   
            ...
            motivation = curiosity_t.squeeze(0).detach().to("cpu").numpy()
        ...
        if done:
            ...
            self._init_goal_buffer(self.state)
        else:
    ...
    def train_model(self):
        state_t, state_next_t, states_seq_t, goal_t, action_t, reward_t, done_t, m = self.experience_replay.sample(self.batch_size, self.model_dqn.device)
        ...
        #train goal creator model, MSE loss
        goal_predicted_t    = self.model_goal_creator(states_seq_t)
        loss_goal           = (goal_t.detach() - goal_predicted_t)**2
        ...
        q_predicted      = self.model_dqn.forward(state_t, goal_predicted_t.detach())
        q_predicted_next = self.model_dqn_target.forward(state_next_t, goal_predicted_t.detach())
